...
export class FileSystemProvider implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider {
	...
	async getChildren(element?: Entry): Promise<Entry[]> {
		...
		let notesFolder = Uri.URI.parse(this.notesLocation);

		if (notesFolder) {
			const children = await this.readDirectory(notesFolder);
			children.sort((a, b) => {
				if (a[1] === b[1]) {
			...
			return children.map(([name, type]) => ({ uri: vscode.Uri.file(path.join(notesFolder.fsPath, name)), type }));
		}
...
export class FileExplorer {
	...
	private notesLocation = String(Notes.getNotesLocation());

	constructor(context: vscode.ExtensionContext) {
		const treeDataProvider = new FileSystemProvider(this.notesLocation);
