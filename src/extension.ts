...
export function activate(context: vscode.ExtensionContext) {
	...
	*/
	let newNoteDisposable = vscode.commands.registerCommand('Notes.new', () => {
		...
		vscode.window.showInputBox({
			...
			let notesFolder = extensionConf.get('notesFolder');
			// set note title
			let fileName: string = `${noteTitle}`;
			// set note path
			let filePath: string = path.join(String(notesFolder), `${fileName.replace(/\:/gi, '')}.md`);
			// set note first line
	...
	*/
	...
	vscode.window.registerTreeDataProvider('notes', new NotesProvider(String(notesFolder)));
	...
	vscode.commands.registerCommand('Notes.delete ', (note: Note) => vscode.window.showInformationMessage(`Successfully deleted ${note.label}.`));
